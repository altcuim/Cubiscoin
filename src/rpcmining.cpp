...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or cubis.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    //if (vNodes.empty())
    //    throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CUBIS is not connected!");

    //if (IsInitialBlockDownload())
    //    throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CUBIS is downloading blocks...");
